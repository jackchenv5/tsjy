...
import { updateAlarmDefinitionApi } from '@/apis/sawingMachineAlarm.js'
...
const handleUpdate = async () => {
  await formRef.value.validate(async (valid) => {
    if (valid) {
      await updateAlarmDefinition()
    }
  })
}
const loadingUpdateAlarmDefinition = ref(false)
const updateAlarmDefinition = async () => {
  if (loadingUpdateAlarmDefinition.value) {
    return
  }
  loadingUpdateAlarmDefinition.value = true
  try {
    await updateAlarmDefinitionApi({
      id: formData.value['id'],
      connectorInstance: formData.value['connectorInstance'],
      connectionName: formData.value['connectionName'],
      dataPoint: formData.value['dataPoint'],
    ...
    loadingUpdateAlarmDefinition.value = false
  }
}
// endregion
</script>
    ...
    >
      ...
      <ElFormItem>
        <ElButton
          type="primary"
          @click="handleUpdate"
          :loading="loadingUpdateAlarmDefinition"
        >
          更新
        </ElButton>
      </ElFormItem>
