...
import { getMotorBindingApi, updateMotorBindingApi } from '@/apis/sawingMotor.js'
...
import UpdateMotorThreshold from '@/components/UpdateMotorThreshold.vue'
...
const showUpdateMotorConfirm = ref(false)
const updateMotorClick = (row) => {
  currentBinding.value = JSON.parse(JSON.stringify(row)) 
  showUpdateMotorConfirm.value = true
}

const onUpdateMotorFinished = async () => {
  await getMotorBindingData()
}
...
// const onUpdateMotorConfirmed = async () => {
//   loadingUpdateMotorBinding.value = true
//   try {
//     const binding = {
//       id: currentBinding.value['id'],
...
<template>
  <div class="container">
    ...
    >
      <ElTableColumn label="序号" type="index" width="55"></ElTableColumn>
      <ElTableColumn label="绑定类型" prop="bindingType" min-width="75">
        <template #default="{ row }">
          {{ getBindingTypeString(row['bindingType']) }}
        </template>
      </ElTableColumn>
      <ElTableColumn label="连接器实例" prop="connectorInstance" width="95">
      </ElTableColumn>
      <ElTableColumn label="连接名称" prop="connectionName" min-width="100">
      </ElTableColumn>
      <ElTableColumn label="数据点名称" prop="dataPoint" width="95">
      </ElTableColumn>
      <ElTableColumn label="正常上阈值" prop="max" min-width="90">
      </ElTableColumn>
      <ElTableColumn label="正常下阈值" prop="min" min-width="90">
      </ElTableColumn>
      <ElTableColumn label="警告上阈值" prop="maxWarning" min-width="90">
      </ElTableColumn>
      <ElTableColumn label="警告下阈值" prop="minWarning" min-width="90">
      </ElTableColumn>
      <ElTableColumn label="报警上阈值" prop="maxAlarm" min-width="90">
      </ElTableColumn>
      <ElTableColumn label="报警下阈值" prop="minAlarm" min-width="90">
      </ElTableColumn>
      <ElTableColumn label="变量名称" prop="name" min-width="200">
      </ElTableColumn>      
      <ElTableColumn width="180">
        <template #default="{ row }">
          <ElButtonGroup size="small">
            <ElButton type="danger" plain @click="updateMotorClick(row)">
              阈值
            </ElButton>
  ...
  <UpdateMotorThreshold
    v-model="showUpdateMotorConfirm"
    :motor = 'currentBinding'
    @finished="onUpdateMotorFinished"
  >
  </UpdateMotorThreshold>
