...
import ProductionHistoryTable from '@/components/ProductionHistoryTable.vue'
...
<template>
  <ContentLayout>
    <template #header>生产历史</template>
    <template #default>
      <FacilityList @change="onFacilityChange"></FacilityList>
      <div class="main" :key="currentFacility?.id">
        <div class="header">
          <ElDatePicker
            style="width: 24rem"
            type="daterange"
        ...
        <div class="content" v-if="currentFacility">
          <ProductionStatistics
            class="statistics"
            v-model="loadChartData"
          ></ProductionStatistics>
          <ProductionHistoryTable class="table" v-model="loadTableData">
          </ProductionHistoryTable>
        </div>
        <ElEmpty class="content" v-else></ElEmpty>
      </div>
    </template>
  </ContentLayout>
</template>
